...
import { getAPI } from "./graphql/queries";
import { updateAPI } from "./graphql/mutations";
...
export default function APIUpdateForm(props) {
  const {
    id: idProp,
    aPI: aPIModelProp,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = aPIRecord
      ? { ...initialValues, ...aPIRecord }
      : initialValues;
    setName(cleanValues.name);
    setDescription(cleanValues.description);
    setEndpoint_url(cleanValues.endpoint_url);
    setMethod(cleanValues.method);
    setCategory(cleanValues.category);
    setStatus(cleanValues.status);
    setFree_requests_limit(cleanValues.free_requests_limit);
    setPrice_per_request(cleanValues.price_per_request);
    setExample_request(
      typeof cleanValues.example_request === "string" ||
        cleanValues.example_request === null
        ? cleanValues.example_request
        : JSON.stringify(cleanValues.example_request)
    );
    setExample_response(
      typeof cleanValues.example_response === "string" ||
        cleanValues.example_response === null
        ? cleanValues.example_response
        : JSON.stringify(cleanValues.example_response)
    );
    setHeaders_required(cleanValues.headers_required ?? []);
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getAPI.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getAPI
        : aPIModelProp;
  ...
  React.useEffect(resetStateValues, [aPIRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          status: status ?? null,
          free_requests_limit: free_requests_limit ?? null,
          price_per_request: price_per_request ?? null,
        ...
        try {
          ...
          await client.graphql({
            query: updateAPI.replaceAll("__typename", ""),
            variables: {
              input: {
                id: aPIRecord.id,
                ...modelFields,
      ...
      {...getOverrideProps(overrides, "APIUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || aPIModelProp)}
